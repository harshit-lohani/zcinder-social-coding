...
function Friends() {
  ...
  const getFriendsList=async event=>{
    ...
    getDoc(DocRef).then(response=>{
      console.log(response._document.data.value.mapValue.fields.friends.arrayValue.values);
      setFriendList(response._document.data.value.mapValue.fields.friends.arrayValue.values);
